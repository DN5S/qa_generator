...
## Log Rule
* `INFO`: 시스템이 정상적으로 작동하고 있음을 알리는 핵심 이벤트만 기록한다. (예: "로깅 시스템 초기화 완료", "파일 처리 시작", "성공적으로 JSON 저장 완료") 이 레벨에서는 절대 민감한 데이터나 상세한 내부 경로를 노출해서는 안 된다. **모든 일반적인 실행은 `INFO` 레벨을 기본으로 한다.**
* `DEBUG`: 특정 문제의 원인을 파악하기 위해 개발자가 직접 실행할 때만 사용한다. 이 레벨에서는 의도적으로 상세한 변수 값, 함수 흐름, 프롬프트 내용 일부 등 민감할 수 있는 정보가 기록된다. `DEBUG` 로그 파일은 분석 후 즉시 파기하거나 안전한 장소에 보관해야 한다.
* **평시:** 항상 `--log-level INFO` 또는 기본값으로 스크립트를 실행한다.
* **문제 발생 시:** 문제가 발생한 특정 파일에 대해서만 `--log-level DEBUG` 옵션을 사용하여 재실행하고, 생성된 로그를 분석한다.
* **로그 파일 관리:** `logs/` 디렉토리에 생성된 파일들, 특히 `DEBUG` 레벨로 생성된 로그는 로컬 시스템 외부로 절대 전송해서는 안 되며, 주기적으로 삭제하는 것을 원칙으로 한다.
